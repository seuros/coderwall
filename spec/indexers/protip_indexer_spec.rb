RSpec.describe ProtipIndexer do
  ...
  describe '#store' do
    it 'should add a users protip to the search index' do
      ...
      expect(Protip.search_by_string('this content').count).to eq(1)
    end
    ...
    it 'should not add a users protip to search index if user is banned' do
      ...
      expect(Protip.search_by_string('Some title').count).to eq(0)
    end
  end
  ...
  describe '#remove' do
    it 'should remove a users protip from search index' do
      ...
      expect(Protip.search_by_string('that troll').count).to eq(1)
      ProtipIndexer.new(protip).remove
      expect(Protip.search_by_string('that troll').count).to eq(0)
    end
  end
end